...
void StarLayers::initializeGL(GLuint program, int quantity) {
  ...
  std::uniform_real_distribution<float> distPosX(-0.9f, 0.3f);
  std::uniform_real_distribution<float> distPosY(1.0f-2.0/30, -1.0f+2.0/30);
  std::uniform_real_distribution<float> distIntensity(0.5f, 1.0f);

  for (auto &&[index, layer] : iter::enumerate(m_starLayers)) {
    layer.m_pointSize = 8.0f / (1.0f + index);
    ...
    for ([[maybe_unused]] auto i : iter::range(0, layer.m_quantity)) {
      data.emplace_back(distPosX(re), distPosY(re), 0);
...
void StarLayers::update(const Player &player, float deltaTime) {
  for (auto &&[index, layer] : iter::enumerate(m_starLayers)) {
    auto layerSpeedScale{1.0f / (index + 2.0f)};
    layer.m_translation -= glm::vec2{0, 1.6f * deltaTime * layerSpeedScale};
